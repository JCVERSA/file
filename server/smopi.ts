...
- Keep output nicely formatted using Markdown headings, bold text, and bullet points.`;
  ...
  try {
    ...
    console.error('Smopi Gemini Agent error:', err);
    // Graceful fallback to local command engine if API rate-limit / invalid key occurs
    const fallback = processLocalCommand(userMessage, shareDir);
    fallback.text = `*(Notice: Switched to Smopi offline mode due to API issue: ${err.message || 'Check GEMINI_API_KEY'}. I processed your request using local commands.)*\n\n` + fallback.text;
    return fallback;
  }
}
