...
const DEFAULT_SOURCES: PromptSource[] = [
  {
    ...
    description: 'Reference files in shared folder',
    icon: Folder,
    attach: true
  },
  { key: 'docs', name: 'Documents', description: 'Read notes, pdfs, txts', icon: FileText },
  { key: 'search', name: 'Search files', description: 'Search keywords & contents', icon: Search }
];

const DEFAULT_COMMANDS: PromptCommand[] = [
  { key: 'summarize', name: '/summarize', description: 'Summarize shared workspace files' },
  { key: 'organize', name: '/organize', description: 'Categorize and inspect files' },
  { key: 'draft', name: '/draft', description: 'Create a new file in workspace' },
  { key: 'explain', name: '/explain', description: 'Explain contents or code' },
  { key: 'tasks', name: '/tasks', description: 'Generate action tasks from files' }
];

const DEFAULT_MODELS: PromptModel[] = [
  { key: 'gemini-3.8-flash', name: 'Gemini 3.8 Flash', tag: 'Fast & Smart' },
  { key: 'gemini-2.5-pro', name: 'Gemini Pro', tag: 'Deep Reasoning' },
  { key: 'offline-agent', name: 'Workspace Agent', tag: 'Local' }
];

const DEFAULT_EFFORTS = ['Fast', 'Balanced', 'Deep', 'Max'];
...
export interface PromptBarProps {
  ...
  busy?: boolean;
  onSend?: (text: string, details: { attachments: string[]; model: PromptModel; effort: string }) => void;
  onStop?: () => void;
  onAttach?: () => Promise<string[] | string | void> | string[] | string | void;
  onDictate?: () => Promise<string | void> | string | void;
  background?: string;
  color?: string;
...
export default function PromptBar({
  ...
  const latest = useRef<{
    ...
    onDictate?: typeof onDictate;
    onEffortChange?: typeof onEffortChange;
  }>({});
  ...
  const onEffortKey = (e: React.KeyboardEvent<HTMLDivElement>) => {
    const step =
